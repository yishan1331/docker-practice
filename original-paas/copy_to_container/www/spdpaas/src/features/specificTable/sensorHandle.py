...
#Yishan 05292019 import func to without ignoring cases
from sqlalchemy import Table, text, select, and_, or_, asc, desc, case, true, literal, distinct, update, between, TIMESTAMP, func
...
#=======================================================
#{{{
from ..customizedHandle import trigger_specific_program
from apiPortal import app
from dbModel.iot import Sensor
import globalvar
import modules
# import web_utility
...
def retrieve_table(SYSTEM,sensor_raw_table):
    status = True
    try:
        DbSession,metadata,engine= app.getDbSessionType(system=SYSTEM)
        ...
        if len(result) == 0:
            ...
            thisid_existed,catch_dbname = modules.retrieve_noumenon_dbname(metadata,sess,result[0]["noumenon_type"],result[0]["noumenon_id"])
        ...
        status = False
        result = app.catch_exception(e,sys.exc_info(),SYSTEM)
        ...
        for table in metaRaw.tables.values():
            if table.name == senTableName:
                for column in table.c:
                    ...
                    if modules.check_user_defined_type(column):
...
def qrydel_backup_temporary_tabledata(SYSTEM,sqlfilename,quarter,setquery):
    ...
    try:
        DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(system=SYSTEM,dbName="iot",forRawData="postgres")
        ...
        if not modules.retrieve_table_exist(metaRaw,sqlfilename,SYSTEM):
...
def get_redis_sensor_IDKEY_ornot_create(SYSTEM,sensorID):
    ...
    try:
        ...
        dbRedis,_,result = app.getDbSessionType(system=SYSTEM,dbName=redis_db,forRawData="redis")
        if dbRedis is None:
            #表示連接資料庫有問題
            return False,result
        ...
        print "~~~eee~~~~"
        print e
        return False,app.catch_exception(e,sys.exc_info(),SYSTEM)
#}}}

# #=======================================================
# # API: /api/IOT/1.0/myps/Sensor/ApiKeys
# # Definition: 提供使用者拿取各sensor的apikeys
# # FOR MYSQL+POSTGRES
# #=======================================================
# #{{{ SENSOR_API.route('/api/<SYSTEM>/1.0/myps/Sensor/ApiKeys', methods = ['POST']), 
# @SENSOR_API.route('/api/<SYSTEM>/1.0/myps/Sensor/ApiKeys',  methods = ['POST'])
# def sensor_get_apikeys(SYSTEM):
#     #{{{APIINFO
#     """
#     {
#         "API_application":"提供使用者拿取不同感測器的apikeys服務",
...
#             "DB": "MYSQL+POSTGRES",
#             "System": "IOT",
#             "apiKeys": {"work_code_use":"","mould_series_no_use":"","mould_series_no_abn":"","material_batch_no_use":"","material_batch_no_abn":""}
#         }
#     } 
#     """
#     #}}}
...
#     reqdataDict = modules.AdjustDataFormat().format(reqdataDict)
...
#     try:
#         sess,DbSession,metadata,engine= app.getDbSessionType(system=SYSTEM)
#         if DbSession is None:
#             #表示連接資料庫有問題
...
#             #查詢該ID是否存在
#             thisIDexisted,sensorDict = retrieve_table(tableName)
#             if not thisIDexisted:
...
#             #=======RSA=======
#             RSA = modules.RSAPrpcrypt(SYSTEM)
#             en_result = RSA.encrypt_by_public_key(dbName+"@"+tableName)

#             #=======AES=======
#             # aes_key = globalvar._SYSTEM_SECRETKEY[SYSTEM]
#             # AES_IV = tableName
#             # AES = modules.AESPrpcrypt(aes_key,AES_IV,SYSTEM)
#             # status,en_result = AES.encrypt(dbName)
#             # if not status:
#             #     dicRet["Response"] = en_result
#             #     return jsonify(**dicRet)

#             apiKeys[tableName] = en_result
#             # RSA = modules.RSAPrpcrypt(SYSTEM)
#             # de_result = RSA.decrypt_by_private_key(apikey)
#         else:
#             dicRet["apiKeys"] = apiKeys
#             err_msg = "ok"
...
def sensor_register(SYSTEM):
    ...
    try:
        ...
        thisid_existed,dbName = modules.retrieve_noumenon_dbname(metadata,sess,noumenon_type,noumenon_id)
        if not thisid_existed:
        ...
        if dbName is not None:
            retrieve_status,retrieve_result = modules.retrieve_database_exist(SYSTEM, dbName=dbName, forRawData="postgres")
            ...
            for i in sensor_attr:
                ...
                if not modules.check_post_parameter_exist(i,post_parameter):
            ...
            if err_msg == "ok":
                ...
                resultRaw ,mesgRaw = modules.create_table(table_name=senRawTable, dbName=dbName, attrList=sensor_attr, table_comment=name, system=SYSTEM)
...
def sensor_post_rows(SYSTEM,sensor_raw_table):
    ...
    try: 
        DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(system=SYSTEM,dbName=dbName,forRawData="postgres")
        if DbSessionRaw is None:
            #表示連接資料庫有問題
        ...
        reqdataDict = modules.ConvertData().convert(reqdataDict)
        #檢查postdata格式是否為陣列
...
def sensor_post_rows_v2(SYSTEM,sensor_raw_table):
    ...
    if trigger_php and err_msg == "ok":
        ...
        trigger_obj = {"table":senTableName,"data":reqdataDict,"upload_at":trigger_settime}
        trigger_specific_program(SYSTEM,selfUse=True,useThread=True,languages="php",programName="/var/www/html/IOT/mes_modules/mes_modules.php",programData=json.dumps(modules.ConvertData().convert(trigger_obj)))
...
def testtimerPost(sensor_raw_table):
    ...
    dicRet = app.preProcessRequest(request,system="IOT")
    ...
    try:
        DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(system="IOT",dbName="site2",forRawData="postgres")
        if DbSessionRaw is None:
            #表示連接資料庫有問題
            dicRet["Response"] = engineRaw
            return jsonify( **dicRet)
...
def sensor_qry_single_row(SYSTEM,querySingle, sensor_raw_table):
    ...
    try: 
        DbSessionRaw,_,engineRaw = app.getDbSessionType(system=SYSTEM,dbName=dbName,forRawData="postgres")
        if DbSessionRaw is None:
            #表示連接資料庫有問題
            dicRet["Response"] = engineRaw
            return jsonify( **dicRet)
        ...
        for row in sessRaw.execute(sqlStr):
            drow = modules.AdjustDataFormat().format(dict(row.items()))
            recLast.append(drow)
...
# FOR MYSQL+POSTGRES
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/myps/Sensor/SqlSyntax/<sensor_raw_table>', methods = ['POST']), 
...
def sensor_qry_sqlsyntax(SYSTEM,sensor_raw_table):
    ...
    try:
        ...
        if where != "":
            ...
            wheretoStrDict = modules.ConvertData().convert(where)
            symbolstoStrDict = modules.ConvertData().convert(symbols)
            #unicode dict change to str dict
            ...
            if str(orderby[0]) != "asc" and str(orderby[0]) != "desc":
                ...
                for i in range(1,len(orderby)):
                    if orderby[i] in fieldList:
                        ...
                        errpostorderbyparameter.append(err_msg)
                    orderbyAY = modules.ConvertData().convert(orderbyAY)
        ...
        if intervaltime != "":
            ...
            for timekey,timevalue in intervaltimetoStrDict.items():
                ...
                for timestr in timevalue:
                    if not modules.VerifyDataStrLawyer(timestr).verify_date():
...
# FOR MYSQL+POSTGRES
#=======================================================
# {{{ app.route('/api/<SYSTEM>/2.0/myps/Sensor/SqlSyntax', methods = ['POST']), 
...
def sensor_qry_sqlsyntax_v2(SYSTEM):
    ...
    '''
    {
        ...
        "API_postData":{
            ...
            "precautions": {
                ...
                "注意事項8":"where子查詢的symbol可以為['in'='in_and','notin'='notin_and','in_or','notin_or']，可自行選擇and或or",
    ...
    action_status,action_result = modules.ApiSqlsyntaxActions({"request_data":reqdataDict["condition_1"],"isjoin":False}).check_post_parameter()
    ...
    try:
        ...
        sqlsyntax_actions_status,sqlsyntax_actions_result = modules.sqlsyntax_params_actions(action_result, "condition_1", reqdataDict, sessRaw, metaRaw, "postgres", False, SYSTEM)
...
# FOR MYSQL+POSTGRES
#=======================================================
# {{{ app.route('/api/<SYSTEM>/2.5/myps/Sensor/SqlSyntax', methods = ['POST']), 
...
# FOR MYSQL+POSTGRES
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/myps/Sensor/SqlSyntax/JoinMultiTable', methods = ['POST']), 
...
def commonuse_qry_joinmultitable(SYSTEM):
    ...
    try:
        ...
        try:
            ...
            join = modules.ConvertData().convert(join)
            ...
            for i in join[join.keys()[0]]:
                response, msg, key, value = modules.adjust_dict_for_joinapi(metadata=metaRaw,tables=tables,masterKey=joinkeyList0,data=i,joinkeyList=joinkeyList,joincolumnList=joincolumnList,selectDB="postgres",system=SYSTEM)
                if not response:
                    dicRet["Response"] = msg
                    return jsonify( **dicRet)
            else:
...
def commonuse_qry_joinmultitable_v2(SYSTEM):
    ...
    '''
    {
        ...
        "API_postData":{
            ...
            "precautions": {
                ...
                "注意事項7":"where若有子查詢需求，value必須為subcondition_x,ex:({'where':{'key':['subcondition_1']},'symbol':{'key':['in']}})，詳例查看example4,5,6,7",
                "注意事項8":"where子查詢查詢回來的欄位必須只有一個",
                "注意事項9":"where子查詢的symbol可以為['in'='in_and','notin'='notin_and','in_or','notin_or']，可自行選擇and或or",
                "注意事項10":"This version of MariaDB doesn't yet support 'LIMIT & IN/ALL/ANY/SOME subquery"
            },
            "example":[
                {
                    "condition_1": {
    ...
    #先查key:condition_1的條件
    action_status,action_result = modules.ApiSqlsyntaxActions({"request_data":reqdataDict["condition_1"],"isjoin":True}).check_post_parameter()
    if not action_status:
        dicRet["Response"] = action_result
        return jsonify( **dicRet)
    ...
    try:
        ...
        try:
            ...
            sqlsyntax_actions_status,sqlsyntax_actions_result = modules.join_sqlsyntax_params_actions(action_result,int("condition_1".split("_")[1]),reqdataDict,sessRaw,metaRaw,"postgres",True,SYSTEM)
            if not sqlsyntax_actions_status:
                dicRet["Response"] = sqlsyntax_actions_result
                return jsonify( **dicRet)
...
def sensor_delete(SYSTEM):
    ...
    try:
        ...
        deleteResult = modules.delete_tables([senTableName],dbName,SYSTEM)
...
# #=======================================================
# #未改
# # API: '/api/IOT/1.0/myps/MultiSensor/Rows/TimeInterval'
# # Definition: 提供一次查詢所有機台的最新資訊(指定時間區段)，postgresql-sensor
# # FOR MYSQL+POSTGRES
# #=======================================================
# #{{{ SENSOR_API.route('/api/IOT/1.0/myps/MultiSensor/Rows/TimeInterval', methods = ['GET'])
# @SENSOR_API.route('/api/IOT/1.0/myps/MultiSensor/Rows/TimeInterval', methods = ['GET'])
# def multi_sensor_latestinfo_timeinterval():
#     #{{{APIINFO
#     '''
#     {
#         "API_application":"提供一次查詢所有機台的最新資訊(指定時間區段)，postgresql->sensor",
#         "API_parameters":{"uid":"使用者帳號","timeStart":"開始時間(格式:%Y-%m-%d %H:%M:%S)","timeEnd":"結束時間(格式:%Y-%m-%d %H:%M:%S)"},
#         "API_message_parameters":{
#             "PsqlSensorData":"JSON",
#             "DB":"string"
#         },
#         "API_example":{
#             "APIS": " GET /api/IOT/1.0/myps/MultiSensor/Rows/TimeInterval",
#             "BytesTransferred": 191,
#             "OperationTime": "0.065",
#             "Response": "ok",
#             "PsqlSensorData": {
#                 "headmachb45": {
#                     "fwdsafety": false, "upload_at": "2019-12-13 18:10:11", "short": false, "backsafety": false, "airpressure": true, "countno": 52262, "lubroil": true, "overload": false, "power": true, "materialend": false, "oilpressure": false, "safetydoor": false, "oiloverflow": false, "tablefloat": false, "inverter": false, "countend": false
#                 }
#             },
#             "DB":"MYSQL+POSTGRES"
#         }
...
#     uri_parameter = ["uid","timeStart","timeEnd"]
#     result, result_msg = modules.check_uri_parameter_exist(request,uri_parameter)
#     if not result:
#         dicRet["Response"] = result_msg
#         return jsonify( **dicRet)

#     start = request.args.get("timeStart")
#     end = request.args.get("timeEnd")
#     if not (modules.VerifyDataStrLawyer(start).verify_date() and modules.VerifyDataStrLawyer(end).verify_date()):
#         err_msg = "timeStart date str : '{}' or timeEnd date str : '{}' is illegal".format(start,end)
#         dicRet["Response"] = err_msg
#         return jsonify( **dicRet)
...
#     pgSensorData = {}
#     try:
#         DbSession,_,engine= app.getDbSessionType(system="IOT")
#         if DbSession is None:
#             #表示連接資料庫有問題
#             err_msg = '<sensor_mysql_timeinterval_latestinfo>Unexpected error: {}'.format(engine)
#             app.logger_handler('IOT','Error', err_msg)
#             dicRet["Response"] = err_msg
#             return jsonify( **dicRet)

#         sess = DbSession()

#         #抓取sensor表裡所有機台ID，以此去postgres抓資料
#         for qrybacksensor in sess.query(Sensor.senID,Sensor.dbName):
#             if not sensor_backdata.get(qrybacksensor[1]):
#                 sensor_backdata[qrybacksensor[1]] = [qrybacksensor[0]]
#             else:
#                 sensor_backdata[qrybacksensor[1]].append(qrybacksensor[0])

#         #抓取sensor表裡所有機台ID，以此去postgres抓資料
#         backdata_dbname = sensor_backdata.keys()
#         backdata_tablename = sensor_backdata.values()

#         mesg = "ok"
#         #抓取postgres device表最新一筆資料
#         mesg = "error"
#         for i in range(len(backdata_dbname)):
#             try: 
#                 sessRaw,DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(system="IOT",dbName=backdata_dbname[i],forRawData="postgres")
#                 if DbSessionRaw is None:
#                     #表示連接資料庫有問題
#                     err_msg = '<sensor_postgresql_timeinterval_latestinfo>Unexpected error: {}'.format(engine)
#                     app.logger_handler('IOT','Error', err_msg)
#                     dicRet["Response"] = err_msg
#                     return jsonify( **dicRet)
...
#                 for j in range(len(backdata_tablename[i])):
#                     pgSensorList = []
#                     sensorTable = Table(backdata_tablename[i][j] , metaRaw, autoload=True)
#                     for row in sessRaw.query(sensorTable).filter(getattr(sensorTable.c,globalvar.UPLOADTIME[globalvar.SERVERIP]["postgres"]).between(start,end)).order_by(desc(getattr(sensorTable.c,globalvar.UPLOADTIME[globalvar.SERVERIP]["postgres"]))):
#                         drow = modules.AdjustDataFormat().format(row._asdict())
#                         pgSensorList.append(drow)
#                     pgSensorData[backdata_tablename[i][j]] = pgSensorList
#                 dicRet["PsqlSensorData"] = pgSensorData
#                 mesg = "ok" 
#             except:
#                 mesg = '<sensor_postgresql_timeinterval_latestinfo>Unexpected error: {}'.format(sys.exc_info()) 
#                 app.logger_handler('IOT','Error', mesg)
#             finally:
...
#                     engineRaw.dispose()
#     except:
#         mesg = '<sensor_mysql_timeinterval_latestinfo>Unexpected error: {}'.format(sys.exc_info()) 
#         app.logger_handler('IOT','Error', mesg)
#     finally:
#         if 'DbSession' in locals().keys() and DbSession is not None:
#             sess.close()
...
# #=======================================================
# #未改
# # API: '/api/IOT/1.0/myps/MultiSensor/LimitRows/LatestInfo'
# # Definition: 提供一次查詢所有機台的最新資訊(指定筆數) postgresql-sensor
# # FOR MYSQL+POSTGRES
# #=======================================================
# #{{{ SENSOR_API.route('/api/IOT/1.0/myps/MultiSensor/LimitRows/LatestInfo', methods = ['GET'])
# @SENSOR_API.route('/api/IOT/1.0/myps/MultiSensor/LimitRows/LatestInfo', methods = ['GET'])
# def multi_sensor_latestinfo_limitrows():
#     #{{{APIINFO
#     '''
#     {
#         "API_application":"提供一次查詢所有機台的最新資訊(指定筆數)，postgresql->sensor",
#         "API_parameters":{"uid":"使用者帳號","limit":"筆數"},
#         "API_message_parameters":{"PsqlSensorData":"JSON","DB":"string"},
#         "API_example":{
#             "APIS": " GET /api/IOT/1.0/myps/MultiSensor/LimitRows/LatestInfo",
#             "BytesTransferred": 191,
#             "OperationTime": "0.065",
#             "Response": "ok",
#             "PsqlSensorData": {
...
#                     {
#                         "status": "E",
#                         "upload_at": "2020-04-22 11:39:12",
#                         "uid": "user",
#                         "workid": "F12106A0048",
#                         "wirewgt": 538.0,
#                         "wireid": "WE_I201910026-002"
#                     }
#                 ]
#             },
#             "DB":"MYSQL+POSTGRES"
...
#         if DbSession is None:
#             #表示連接資料庫有問題
#             err_msg = '<sensor_mysql_limitrows_latestinfo>Unexpected error: {}'.format(engine)
#             app.logger_handler('IOT','Error', err_msg)
#             dicRet["Response"] = err_msg
#             return jsonify( **dicRet)
...
#                 if DbSessionRaw is None:
#                     #表示連接資料庫有問題
#                     err_msg = '<sensor_postgresql_limitrows_latestinfo>Unexpected error: {}'.format(engine)
#                     app.logger_handler('IOT','Error', err_msg)
#                     dicRet["Response"] = err_msg
#                     return jsonify( **dicRet)
...
#                     pgSensorList = []
#                     sensortable = Table(backdata_tablename[i][j] , metaRaw, autoload=True)
#                     for row in sessRaw.query(sensortable).order_by(desc(getattr(sensortable.c,globalvar.UPLOADTIME[globalvar.SERVERIP]["postgres"]))).limit(limit):
#                         drow = modules.AdjustDataFormat().format(row._asdict())
#                         pgSensorList.append(drow)
#                     pgSensorData[backdata_tablename[i][j]] = pgSensorList
#                 dicRet["PsqlSensorData"] = pgSensorData
...
#             except:
#                 #debug 04072017
#                 mesg = '<sensor-sensor_postgresql_limitrows_latestinfo>Unexpected error: {}'.format(sys.exc_info()) 
#                 app.logger_handler('IOT','Error', mesg)
#             finally:
#                 if DbSessionRaw is not None:
#                     sessRaw.close()
...
#                     engineRaw.dispose()
#     except:
#         mesg = '<sensor-sensor_mysql_limitrows_latestinfo>Unexpected error: {}'.format(sys.exc_info()) 
#         app.logger_handler('IOT','Error', mesg)
#     finally:
#         if 'DbSession' in locals().keys() and DbSession is not None:
#             sess.close()
...
#=======================================================
#{{{ @COMMONUSE_IOT_API.route('/api/<SYSTEM>/1.0/ps/CommonUse/BackUp/<sensor_raw_table>',  methods = ['GET'])
@SENSOR_API.route('/api/<SYSTEM>/1.0/ps/CommonUse/BackUp/<sensor_raw_table>',  methods = ['GET'])
@modules.docstring_parameter(ACCESS_SYSTEM_LIST=ACCESS_SYSTEM_LIST)
def sensor_backup_table(SYSTEM,sensor_raw_table):
    ...
    '''
    {
        ...
        "API_example":{
            "APIS": " GET /api/IOT/1.0/ps/CommonUse/BackUp/molduse",
    ...
    try: 
        ...
        try:
            ...
            app.logger_handler(SYSTEM,'Error', '<sensor_backup_restore_table>Unexpected error: {}'.format(e.output))
...
# Date: 09032019@Yishan
#=======================================================
#{{{ @COMMONUSE_IOT_API.route('/api/<SYSTEM>/1.0/ps/CommonUse/BackUpData/<sqlfilename>',  methods = ['GET'])
@SENSOR_API.route('/api/<SYSTEM>/1.0/ps/CommonUse/BackUpData/<sqlfilename>',  methods = ['GET'])
@modules.docstring_parameter(ACCESS_SYSTEM_LIST=ACCESS_SYSTEM_LIST)
def sensor_get_backupdata(SYSTEM,sqlfilename):
    ...
    '''
    {
        ...
        "API_example":{
            "APIS": " GET /api/IOT/1.0/ps/CommonUse/BackUpData/molduse_2018",
            "BytesTransferred": 523,
            "OperationTime":"0.014",
            "Response": "ok",
...
#=======================================================
# API: /api/<SYSTEM>/1.0/myps/Sensor/Create/Default/SensorTable
# Definition: 提供使用者註冊5個預設機台感測資料表(main、emeter、vibMoter、vibBearing、servoD)
# FOR MYSQL+POSTGRES
#=======================================================
#{{{ SENSOR_API.route('/api/<SYSTEM>/1.0/myps/Sensor/Create/Default/SensorTable', methods = ['POST']), 
@SENSOR_API.route('/api/<SYSTEM>/1.0/myps/Sensor/Create/Default/SensorTable',  methods = ['POST'])
@modules.docstring_parameter(ACCESS_SYSTEM_LIST=ACCESS_SYSTEM_LIST)
def create_five_default_sensor_table(SYSTEM):
    #{{{APIINFO
    """
    {
        "API_application":"提供使用者抓取不同感測器的NoumenonID",
    ...
    from dbModel.iot import create_five_device_table
    print "=============@@@@@@@@@@@@@@@=============="
    status,err_msg = create_five_device_table(app,serialID)
...
@SENSOR_API.route('/test/postrows/<device>/<number>',  methods = ['POST'])
def test_sensor_post_rows_v2(device, number, SYSTEM="IOT"):
    err_msg = "error"
    dicRet = app.preProcessRequest(request,system=SYSTEM)
    if SYSTEM not in list(set(globalvar.SYSTEMLIST[globalvar.SERVERIP]).intersection(set(ACCESS_SYSTEM_LIST))):
        dicRet["Response"] = "system:{} has no privillege to use this API".format(SYSTEM)
    ...
    try:
        ...
        if not prikeyisTime:
            ...
            for i in reqdataDict:
                ...
                sessRaw.execute(sqlInsert)
                sessRaw.commit()
        dicRet["InsertOverTime"] = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[::]
        err_msg = "ok"
        ...
        #     updateTable = Table("machine_status_head", metadata,  autoload=True)
        #     sess.execute(updateTable.update().where(getattr(updateTable.c,"device_name") == device.upper()).values({"machine_detail":json.dumps(reqdataDict[0]["machine_detail"])}))
        #     sess.commit()
    ...
    if trigger_php and err_msg == "ok":
        ...
        # print "~~~~trigger_specific_program~~~~~"
        # print datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[::]
        trigger_obj = {"table":senTableName,"data":reqdataDict,"upload_at":trigger_settime,"number":number}
        trigger_specific_program(SYSTEM,selfUse=True,useThread=True,languages="php",programName="/var/www/html/mes_modules/mes_modules.php",programData=json.dumps(modules.ConvertData().convert(trigger_obj)),Temp=True)
        # print "~~~~trigger_specific_program over~~~~~"
        # print datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[::]

    dicRet["Response"] = err_msg
    dicRet["DB"] = "MYSQL+POSTGRES"
    dicRet["FunctionEndTime"] = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[::]
    return jsonify(**dicRet) 
# }}}