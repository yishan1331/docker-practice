...
from sqlalchemy import Table, text, select, and_, or_, asc, desc, case, true, literal, distinct, between, func
import hmac, hashlib
...
def qrydel_backup_temporary_tabledata(SYSTEM,sqlfilename,quarter,setquery):
    ...
    try:
        ...
        if not modules.retrieve_table_exist(metadata,'temporary_'+sqlfilename,SYSTEM):
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/CommonUse/TableData', methods = ['GET']), 
...
def transitionDB_commonuse_get_tabledata(SYSTEM):
    ...
    try:
        ...
        if not modules.retrieve_table_exist(metadata,tableName,SYSTEM):
...
def transitionDB_commonuse_create_table(SYSTEM):
    ...
    try:
        ...
        if modules.retrieve_table_exist(metadata,table_name,SYSTEM):
        ...
        result ,err_msg = modules.create_table(table_name=table_name, attrList=table_attr, table_comment=table_comment, forRawData="mysql",system=SYSTEM)
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/CommonUse/SqlSyntax/<tableName>', methods = ['POST']), 
...
def transitionDB_commonuse_get_sqlsyntax(SYSTEM,tableName):
    ...
    try:
        ...
        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,SYSTEM)
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/<SYSTEM>/2.0/my/CommonUse/SqlSyntax', methods = ['POST']), 
...
def transitionDB_commonuse_get_sqlsyntax_v2(SYSTEM):
    ...
    '''
    {
        ...
        "API_postData":{
            ...
            "parameters":{
                ...
                "fields":{"type":"Array","requirement":"required","directions":"查詢欄位名稱參數","example":"['id','name','z',......]"},
                "where":{"type":"Object","requirement":"required","directions":"查詢條件參數；物件裡的value皆以array型態post，若有特殊or需求請看注意事項3,4,5；若需使用子查詢請看注意事項6,7","example":"{'field':['value',....],.....}"},
                "orderby":{"type":"Array","requirement":"required","directions":"查詢排序參數","example":"['desc(asc)','id','name','z',.....]"},
                "limit":{"type":"Array","requirement":"required","directions":"查詢筆數限制參數；只接受2個數字(第一個數字為從第幾筆開始查，第二個數字為總共要查幾筆)，如需查詢所有資料使用['ALL'],若無設定則預設查100筆","example":"[1,10]"},
    ...
    try:
        ...
        sqlsyntax_actions_status,sqlsyntax_actions_result = modules.sqlsyntax_params_actions(action_result,"condition_1",reqdataDict,sess,metadata,"mysql",False,SYSTEM)
    ...
    try:
        ...
        if tables != "":
            ...
            for i in range(len(tables)):
                if not modules.retrieve_table_exist(metadata,tables[i],SYSTEM):
        ...
        if fields != "":
            ...
            for fieldskey,fieldsvalue in fields.items():
                ...
                desc_table_return = modules.desc_table(metadata,fieldskey,SYSTEM)
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
            ...
            for i in range(len(tables)):
                desc_table_return = modules.desc_table(metadata,tables[i],SYSTEM)
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
                keyList = desc_table_return[2]
        ...
        for i in join[join.keys()[0]]:
            response, msg, key, value = modules.adjust_dict_for_joinapi(metadata=metadata,tables=tables,masterKey=joinkeyList0,data=i,joinkeyList=joinkeyList,joincolumnList=joincolumnList,system=SYSTEM)
            if not response:
                dicRet["Response"] = msg
                return jsonify( **dicRet)
        else:
        ...
        if where != "":
            ...
            for wherekey,wherevalue in wheretoStrDict.items():
                ...
                #desc table schema
                desc_table_return = modules.desc_table(metadata,wherekey,SYSTEM)
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
                keyList = desc_table_return[2]
                schemaDict = desc_table_return[3]
        ...
        if orderby != "":
            ...
            for i in range(1,len(orderby)):
                if i % 2 == 1:
                    ...
                    desc_table_return = modules.desc_table(metadata,orderby[i-1],SYSTEM)
                    desc_table_status = desc_table_return[0]
                    fieldList = desc_table_return[1]
                    keyList = desc_table_return[2]
...
def transitionDB_commonuse_get_sqlsyntax_joinmultitable_v2(SYSTEM):
    ...
    try:
        ...
        sqlsyntax_actions_status,sqlsyntax_actions_result = modules.join_sqlsyntax_params_actions(action_result,"condition_1",reqdataDict,sess,metadata,"mysql",True,SYSTEM)
        if not sqlsyntax_actions_status:
            dicRet["Response"] = sqlsyntax_actions_result
            return jsonify( **dicRet)
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/CommonUse/<tableName>', methods = ['POST'])
...
def transitionDB_commonuse_register(SYSTEM,tableName):
    ...
    try:
        ...
        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,SYSTEM,havetime=False)
        desc_table_status = desc_table_return[0]
        fieldList = desc_table_return[1]
        keyList = desc_table_return[2]
        schemaDict = desc_table_return[3]
            ...
            if not check_data_existed:
                for x in range(len(fieldList)):
                    ...
                    if reqdataDict.get(fieldList[x]) is None:
                        ...
                        postDatatostrList = modules.ConvertData().convert(reqdataDict.get(fieldList[x]))
                                ...
                                if tableName == "user" or fieldList[x] != "id":
                                    ...
                                    if postDatatostrList[thisNum] is not None:
                                        if (not (isinstance(postDatatostrList[thisNum],dict) or isinstance(postDatatostrList[thisNum],float) or isinstance(postDatatostrList[thisNum],int))) and \
                                            ((not modules.VerifyDataStrLawyer(postDatatostrList[thisNum]).verify_json()) and (schemaDict[fieldList[x]] not in ("TIMESTAMP","DATETIME","DATE"))):
                                            #Yishan@09102019 data裡有: 會導致UnicodeDecodeError，所以利用re替代掉
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.5/my/CommonUse/<tableName>', methods = ['POST'])
...
def transitionDB_commonuse_register_v1_5(SYSTEM,tableName):
    ...
    try:
        ...
        result = modules.commonuse_register_integration_myms(reqdataDict, sess, metadata, tableName, SYSTEM, "mysql")
        err_msg = result[0]

        if len(result) == 2:
            dicRet["insertstatus"] = result[1]
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/CommonUse/<tableName>', methods = ['PATCH'])
...
def transitionDB_commonuse_update(SYSTEM,tableName):
    ...
    try:
        ...
        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,SYSTEM,havetime=True)
        desc_table_status = desc_table_return[0]
        fieldList = desc_table_return[1]
        keyList = desc_table_return[2]
        schemaDict = desc_table_return[3]
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.5/my/CommonUse/<tableName>', methods = ['PATCH'])
...
def transitionDB_commonuse_update_v1_5(SYSTEM,tableName):
    ...
    try:
        ...
        result = modules.commonuse_update_integration_myms(reqdataDict, sess, metadata, tableName, SYSTEM, "mysql")
        err_msg = result[0]
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/CommonUse/<tableName>', methods = ['DELETE'])
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.5/my/CommonUse/<tableName>', methods = ['DELETE'])
...
def transitionDB_commonuse_delete_v1_5(SYSTEM,tableName):
    ...
    try:
        ...
        result = modules.commonuse_delete_integration_myms(reqdataDict, sess, metadata, tableName, SYSTEM, "mysql")
        err_msg = result[0]
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/CommonUse/Schema/<tableName>', methods = ['GET'])
@TRANSITIONDB_API.route('/api/<SYSTEM>/1.0/my/CommonUse/Schema/<tableName>', methods = ['GET'])
#@modules.DecoratorCheckUriParameterExist(request,["uid"]) #利用裝飾器檢查
...
def transitionDB_commonuse_backup_mysql_table(SYSTEM,tableName):
    ...
    try:
        ...
        sqldump_cmd = '/usr/bin/mysqldump -uroot -psapido {} {} --single-transaction --skip-tz-utc --where="{} between \'{}\' and \'{}\'"> {}'.format(app.dicConfig.get("DBMYSQLDbname_"+SYSTEM),tableName,globalvar.CREATETIME[globalvar.SERVERIP]["mysql"],start_year,end_year,tableName+'_'+year+'.sql')
        try:
            ...
            if err_msg == "ok": 
                ...
                try:
                    ...
                    app.logger_handler(SYSTEM,'Error', '<commonuse_backup_mysql_data>Unexpected error: {}'.format(d.output))
        except subprocess.CalledProcessError as e:
            err_msg = '<commonuse_backup_mysql_data>Unexpected error: 備份失敗'
            app.logger_handler(SYSTEM,'Error', '<commonuse_backup_mysql_data>Unexpected error: {}'.format(e.output))
...
def transitionDB_commonuse_get_mysql_backupdata(SYSTEM,sqlfilename):
    ...
    try:
        ...
        sql_cmd = '/usr/bin/mysql -uroot -psapido {} < {}'.format(app.dicConfig.get("DBMYSQLDbname_"+SYSTEM),sqlfilename+".sql")
        ...
        app.logger_handler(SYSTEM,'Error', '<commonuse_get_mysql_backupdata>Unexpected error: {}'.format(e.output))
...
# API /session
#=======================================================
# {{{ TRANSITIONDB_API.route('/session', methods = ['GET'])
@TRANSITIONDB_API.route('/v1/session', methods = ['GET'])
def GET_SESS():
    return get_session_id()
# }}}
# {{{ get_session_id() impl, for GET_SESS('/session')
#def get_session_id():
#    #the very FIRST API 'get session id' for accessing SAPIDOSYSTEM PaaS RETful APIs
#    #dicConfig = json.loads(readConfig())
#    dicRet = {}
#    err_msg = "ok"
#    dicRet['session_id'] = "not_a_valid_session"
#    #app.logging.info("{} {} from {}".format(request.method,request.path,request.args.get("user_id","undefined")))
#    attributes = ["uid", "token", "timestamp", "signature"]
#    validArgs,missing_attr = web_utility.check_parameters(request.args, attributes, "SAPIDOSYSTEM")
#    print "~~validArgs~~"
#    print validArgs
#    print "~~missing_attr~~"
#    print missing_attr
...
#    #missing_attr = 'miss'
#    if not validArgs:
#        err_msg = 'missing some parameters:' + missing_attr
#        app.sessionLogger_SAPIDOSYSTEM('Error','in Get Session, []: {}\n'.format(err_msg))
#        app.sessionLogger_SAPIDOSYSTEM('Error','get_session_id() check parameter fail. {}\n'.format(err_msg))
#    else:
...
#                          '&method=GET'
#        print "==============="
#        print request.args['signature']
#        print string_to_sign
#        print app.dicConfig.get('key_loc')
#        print "==============="
#        sig_check_ok, cal_signature = web_utility.check_signature(\
#                                      request.args['signature'], \
#                                      string_to_sign, \
#                                      app.dicConfig.get('key_loc'),
#                                      app.dicConfig.get('log_path'))
#        print "~~sig_check_ok~~"
#        print sig_check_ok
#        print "~~cal_signature~~"
#        print cal_signature
#        if not sig_check_ok:
#            err_msg = 'signature verified failed for user_id={}'.format(request.args['uid'])
#            app.sessionLogger_SAPIDOSYSTEM('Error','get_session_id(): {}\n'.format(err_msg))
#            app.sessionLogger_SAPIDOSYSTEM('Error','client sent request.args:{}\n'.format(request.args))
#            app.sessionLogger_SAPIDOSYSTEM('Error','string_to_sign = {}\n'.format(string_to_sign))
#            app.sessionLogger_SAPIDOSYSTEM('Error','server calcaluted sign is {}\n'.format(cal_signature))
#            app.sessionLogger_SAPIDOSYSTEM('Error','client sent sign is {}\n'.format(request.args['signature'] ))
#
#            hmacSha1 = hmac.new('399A907A433A4C2A675E4EC3F123E463585AF9F9', string_to_sign,hashlib.sha1).digest()
#            print "~~hmacSha1~~"
#            print hmacSha1
#            print type(hmacSha1)
#            strHmacSha1 = hmac.new('399A907A433A4C2A675E4EC3F123E463585AF9F9', string_to_sign,hashlib.sha1).hexdigest()
#            print "~~strHmacSha1~~"
#            print strHmacSha1
#
#            strDebugMsg ="{},client sent request.args:{},client sent signature: {},server expected string_to_sign: {},server intermediate hmac-sha1 result:'',server intermediate hmac-sha1 result in str:{},server calcaluted sign: {}"\
#                                .format(err_msg,\
#                                    request.args,\
#                                    request.args['signature'],\
#                                    string_to_sign,\
...
#                                    strHmacSha1,\
#                                    cal_signature)
#            print type(strDebugMsg)
#            app.sessionLogger_SAPIDOSYSTEM('Error',strDebugMsg)
#            dicRet['debug_msg'] = strDebugMsg
#        else:
#            current_timestamp = request.args['timestamp']
#            #wei@03142017, adding uid pwd process thx DB
#            #pwd = app.dicConfig.get('user').get(request.args['uid'])
#            pwd = "1218test"
#            #app.logging.error('Line 480: uid: {}, passwd: {}\n'.format(request.args['uid'], pwd))
#            token_check_ok, err = authentic_utility.check_token(app.dicConfig.get('key_loc'), \
#                                        app.dicConfig.get('code'),   \
#                                        str(request.args['token']), \
#                                        request.args['uid'], \
#                                        #dicConfig.get('secret'),   \
#                                        pwd, \
#                                        current_timestamp, \
#                                        app.dicConfig.get('timeout'),   \
#                                        app.dicConfig.get('log_path'))
#            print "~~token_check_ok~~"
#            print token_check_ok
#            print "~~err~~"
#            print err
#            if not token_check_ok:
#                err_msg = 'toke check failed for user_id={}, {}'.format(request.args['uid'],err)
#                app.sessionLogger_SAPIDOSYSTEM('Error','get_session_id(): {}\n'.format(err_msg))
#
#
#        if err_msg == "ok":
...
#            #pwd = app.dicConfig.get('user').get(request.args['uid'])
#            pwd = "1218test"
#            #app.logging.error('Line 498: uid: {}, pwd: {}\n'.format(request.args['uid'], pwd))
#            #session_id_str = dicConfig.get('secret') + '*' + current_timestamp
#            session_id_str = pwd + '*' + current_timestamp
#            print "~~session_id_str~~"
#            print session_id_str
#            encrypted_session_id = authentic_utility.encrypt_pgp(app.dicConfig.get('key_loc'), \
#                                                                    session_id_str, \
#                                                                    app.dicConfig.get('log_path') )
#            print "~~encrypted_session_id~~"
#            print encrypted_session_id
#            dicRet['session_id'] = encrypted_session_id
#
#    dicRet['Response'] = err_msg
#    print "~~~~~~~~~~~~~dicRet~~~~~~~~~~~~~~~"
#    print dicRet
