...
import globalvar
import modules
from ..specificTable.sensorHandle import get_redis_sensor_IDKEY_ornot_create, _desc_table
...
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/mix/CommonUse/Operations', methods = ['POST'])
...
def mixingDB_commonuse_register_update_delete(SYSTEM):
    ...
    database_config = {
        "mysql":{
            "post":modules.commonuse_register_integration_myms,
            "patch":modules.commonuse_update_integration_myms,
            "put":modules.commonuse_update_integration_myms,
            "delete":modules.commonuse_delete_integration_myms
        },
        "mssql":{
            "post":modules.commonuse_register_integration_myms,
            "patch":modules.commonuse_update_integration_myms,
    ...
    try:
        for element in reqdataDict:
            ...
            print element
            element_status = []
            if not modules.check_post_parameter_exist(element,["database","operations","datas"]):
                ...
                if element["database"] == "postgres":
                    ...
                    if element["database"] == "mysql":
                        ...
                        if element["operations"][i] == "delete":
                            if not modules.check_post_parameter_exist(element["datas"][i],["keys"]):
            ...
            for db in dbName:
                try:
                    DbSession,metadata,engine= app.getDbSessionType(dbName=db, forRawData=element["database"], system=SYSTEM)
                    ...
                    if element["database"] == "postgres":
                        ...
                        if trigger_php:
                            ...
                            trigger_obj = {"table":table,"data":element["datas"][index],"upload_at":trigger_settime}
                        ...
                        for i in range(len(element["operations"])):
                            if element["database"] == "redis":
                                ...
                                if len(result) == 2:
                                    element_status.append(result[1])
                                else:
                                    element_status.append(result[0])

                except Exception as e:
                    element_status.append(app.catch_exception(e,sys.exc_info(),SYSTEM))
                finally:
                    print "@@@@@@@@@@@@@@@@@@@@@@@@"
