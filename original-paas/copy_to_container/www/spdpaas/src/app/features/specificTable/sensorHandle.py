...
#             #=======RSA=======
#             RSA = RSAPrpcrypt(SYSTEM)
#             en_result = RSA.encrypt_by_public_key(dbName+"@"+tableName)

#             #=======AES=======
#             # aes_key = _SYSTEM_SECRETKEY[SYSTEM]
#             # AES_IV = tableName
#             # AES = AESPrpcrypt(aes_key,AES_IV,SYSTEM)
...
def sensor_post_rows_v2(SYSTEM,sensor_raw_table):
    ...
    if trigger_php and err_msg == "ok":
        ...
        trigger_specific_program(SYSTEM,selfUse=True,useThread=True,languages="php",programName="/var/www/html/IOT/mes_modules/mes_modules.php",programData=json.dumps(ConvertData().convert(trigger_obj)))
...
def commonuse_qry_joinmultitable_v2(SYSTEM):
    ...
    try:
        ...
        try:
            ...
            sqlsyntax_actions_status,sqlsyntax_actions_result = join_modules.sqlsyntax_params_actions(action_result,int("condition_1".split("_")[1]),reqdataDict,sessRaw,metaRaw,"postgres",True,SYSTEM)
            if not sqlsyntax_actions_status:
                dicRet["Response"] = sqlsyntax_actions_result
                return jsonify( **dicRet)
...
#     if not (VerifyDataStrLawyer(start).verify_date() and VerifyDataStrLawyer(end).verify_date()):
...
#     pgSensorData = {}
#     try:
#         DbSession,_,engine= appPaaS.getDbSessionType(system="IOT")
#         if DbSession is None:
#             #表示連接資料庫有問題
#             err_msg = '<sensor_mysql_timeinterval_latestinfo>Unexpected error: {}'.format(engine)
#             appPaaS.logger_handler('IOT','Error', err_msg)
#             dicRet["Response"] = err_msg
#             return jsonify( **dicRet)
...
#             try: 
#                 sessRaw,DbSessionRaw,metaRaw,engineRaw = appPaaS.getDbSessionType(system="IOT",dbName=backdata_dbname[i],forRawData="postgres")
#                 if DbSessionRaw is None:
#                     #表示連接資料庫有問題
...
#                 appPaaS.logger_handler('IOT','Error', mesg)
#             finally:
...
def create_five_default_sensor_table(SYSTEM):
    ...
    print "=============@@@@@@@@@@@@@@@=============="
    status,err_msg = create_five_device_table(appPaaS,serialID)
