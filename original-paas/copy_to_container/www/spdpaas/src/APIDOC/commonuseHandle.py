...
#=======================================================
# {{{ app.route('/api/APIDOC/1.0/CommonUse/Time', methods = ['GET']), 
...
def commonuse_get_time():
    ...
    #}}}
    err_msg = "ok"
    dicRet = app.preProcessRequest(request,system="APIDOC")
    ...
    try:
        ...
        err_msg = app.catch_exception(e,sys.exc_info(),"APIDOC")
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/APIDOC/1.0/my/CommonUse/TableData', methods = ['GET']), 
...
def commonuse_get_tabledata():
    ...
    recList = []
    try:
        DbSession,metadata,engine= app.getDbSessionType(system="APIDOC")
        ...
        if not modules.retrieve_table_exist(metadata,tableName,"APIDOC"):
        ...
        for row in sess.query(query_table).filter(query_field.between(valuestart,valueend)).all():
            ...
            for key,value in drow.items():
                if key=="pwd": #for user to AES_DECRYPT pwd
                    AES_IV = valuestart
                    AES = modules.Prpcrypt(app.dicConfig.get('aes_key'),AES_IV,"APIDOC")
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/APIDOC/1.0/my/CommonUse/SqlSyntax/<tableName>', methods = ['POST']), 
...
def commonuse_get_sqlsyntax(tableName):
    ...
    try:
        ...
        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,"APIDOC")
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/APIDOC/1.0/my/CommonUse/SqlSyntax_', methods = ['POST']), 
...
def commonuse_get_sqlsyntax_():
    ...
    try:
        ...
        sqlsyntax_actions_status,sqlsyntax_actions_result = modules.sqlsyntax_params_actions(action_result,"condition_1",reqdataDict,sess,metadata,"mysql",False,"APIDOC")
    ...
    try:
        ...
        if tables != "":
            ...
            for i in range(len(tables)):
                if not modules.retrieve_table_exist(metadata,tables[i],"APIDOC"):
        ...
        if fields != "":
            ...
            for fieldskey,fieldsvalue in fields.items():
                ...
                desc_table_return = modules.desc_table(metadata,fieldskey,"APIDOC")
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
            ...
            for i in range(len(tables)):
                desc_table_return = modules.desc_table(metadata,tables[i],"APIDOC")
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
                keyList = desc_table_return[2]
        ...
        for i in join[join.keys()[0]]:
            response, msg, key, value = modules.adjust_dict_for_joinapi(metadata=metadata,tables=tables,masterKey=joinkeyList0,data=i,joinkeyList=joinkeyList,joincolumnList=joincolumnList,system="APIDOC")
            if not response:
                dicRet["Response"] = msg
                return jsonify( **dicRet)
        else:
        ...
        if where != "":
            ...
            for wherekey,wherevalue in wheretoStrDict.items():
                ...
                #desc table schema
                desc_table_return = modules.desc_table(metadata,wherekey,"APIDOC")
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
                keyList = desc_table_return[2]
                schemaDict = desc_table_return[3]
        ...
        if orderby != "":
            ...
            for i in range(1,len(orderby)):
                if i % 2 == 1:
                    ...
                    desc_table_return = modules.desc_table(metadata,orderby[i-1],"APIDOC")
                    desc_table_status = desc_table_return[0]
                    fieldList = desc_table_return[1]
                    keyList = desc_table_return[2]
...
def commonuse_get_sqlsyntax_joinmultitable_():
    ...
    try:
        ...
        sqlsyntax_actions_status,sqlsyntax_actions_result = modules.join_sqlsyntax_params_actions(action_result,"condition_1",reqdataDict,sess,metadata,"mysql",True,"APIDOC")
        if not sqlsyntax_actions_status:
            dicRet["Response"] = sqlsyntax_actions_result
            return jsonify( **dicRet)
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/APIDOC/1.0/my/CommonUse/<tableName>', methods = ['POST'])
...
def commonuse_register(tableName):
    ...
    try:
        ...
        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,"APIDOC",havetime=False)
        desc_table_status = desc_table_return[0]
        fieldList = desc_table_return[1]
        keyList = desc_table_return[2]
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/APIDOC/1.0/my/CommonUse/<tableName>', methods = ['PATCH'])
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/APIDOC/1.0/my/CommonUse/<tableName>', methods = ['DELETE'])
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/APIDOC/1.0/my/CommonUse/Schema/<tableName>', methods = ['GET'])
...
# API /session
#=======================================================
# {{{ COMMONUSE_APIDOC_API.route('/session', methods = ['GET'])
@COMMONUSE_APIDOC_API.route('/v1/session', methods = ['GET'])
def GET_SESS():
    return get_session_id()
# }}}
# {{{ get_session_id() impl, for GET_SESS('/session')
#def get_session_id():
#    #the very FIRST API 'get session id' for accessing APIDOC PaaS RETful APIs
...
#    validArgs,missing_attr = web_utility.check_parameters(request.args, attributes, "APIDOC")
#    print "~~validArgs~~"
#    print validArgs
#    print "~~missing_attr~~"
#    print missing_attr
...
#        app.sessionLogger_APIDOC('Error','in Get Session, []: {}\n'.format(err_msg))
#        app.sessionLogger_APIDOC('Error','get_session_id() check parameter fail. {}\n'.format(err_msg))
#    else:
...
#            app.sessionLogger_APIDOC('Error','get_session_id(): {}\n'.format(err_msg))
#            app.sessionLogger_APIDOC('Error','client sent request.args:{}\n'.format(request.args))
#            app.sessionLogger_APIDOC('Error','string_to_sign = {}\n'.format(string_to_sign))
#            app.sessionLogger_APIDOC('Error','server calcaluted sign is {}\n'.format(cal_signature))
#            app.sessionLogger_APIDOC('Error','client sent sign is {}\n'.format(request.args['signature'] ))
#
...
#            app.sessionLogger_APIDOC('Error',strDebugMsg)
