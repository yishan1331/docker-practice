...
#=======================================================
#{{{
from apiPortal import threading, lock, app
import globalvar
import modules
#}}}
...
def subprocess_check_output_program(cmd,SYSTEM):
    try:
        ...
        print "~~~~e~~~~"
        print e.__dict__
        return False,app.catch_exception(e,sys.exc_info(),SYSTEM),e.__dict__
# }}}
...
def create_downloadfiles_idpwd():
    ...
    reqdataList = modules.ConvertData().convert(json.loads(request.data))
...
def trigger_specific_program(SYSTEM,selfUse=False,useThread=False,languages=None,programName=None,programData=None,Temp=False):
    ...
    try:
        if useThread:
            ...
            process = subprocess_check_output_program(modules.ConvertData().convert(cmd),SYSTEM)
            # print "~~~~process~~~~"
            # print process
            # print "~~~~type process~~~~"
            # print type(process)
...
#=======================================================
# {{{def iot_redis_device_keys_init(SYSTEM,sensorID)
...
def iot_redis_device_keys_init(SYSTEM, selfUse=False):
    ...
    try:
        ...
        dbRedis,_,_ = app.getDbSessionType(system=SYSTEM,dbName=redis_db,forRawData="redis")
        if dbRedis is None:
            return
