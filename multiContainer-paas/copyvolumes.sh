...
docker exec -it celeryworker /etc/init.d/rsyslog start
docker exec -it celeryworker crontab /var/spool/cron/crontabs/root
docker exec -it celeryworker /etc/init.d/cron start
