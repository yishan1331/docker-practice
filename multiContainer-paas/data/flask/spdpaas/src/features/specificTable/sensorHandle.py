...
#=======================================================
#{{{ @SENSOR_API.route('/api/<SYSTEM>/1.0/ps/BackUp/<sensor_raw_table>',  methods = ['GET'])
@SENSOR_API.route('/api/<SYSTEM>/1.0/ps/BackUp/<sensor_raw_table>',  methods = ['GET'])
@modules.docstring_parameter(ACCESS_SYSTEM_LIST=ACCESS_SYSTEM_LIST)
def sensor_backup_table(SYSTEM,sensor_raw_table):
    ...
    '''
    {
        ...
        "API_example":{
            "APIS": " GET /api/IOT/1.0/ps/BackUp/molduse",
...
def sensor_get_backupdata(SYSTEM,sqlfilename):
    ...
    '''
    {
        ...
        "API_example":{
            "APIS": " GET /api/IOT/1.0/ps/BackUpData/molduse_2018",
            "BytesTransferred": 523,
            "OperationTime":"0.014",
            "Response": "ok",
...
def get_tables_size(SYSTEM,db_name):
    ...
    for timestr in (start,end):
        if not modules.VerifyDataStrLawyer(timestr).verify_date(is_date=True):
    ...
    try: 
        #create Raw table in postgresql DB
        DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(system=SYSTEM,dbName=db_name,forRawData="postgres")
        if DbSessionRaw is None:
            #表示連接資料庫有問題
            dicRet["Response"] = engineRaw
            return jsonify( **dicRet)
        ...
        metaRaw.reflect()
        tableList = modules.ConvertData().convert(metaRaw.tables.keys())
        print tableList
