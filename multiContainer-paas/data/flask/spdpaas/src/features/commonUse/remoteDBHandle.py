...
from sqlalchemy import Table, text, select, and_, or_, asc, desc, case, true, literal, distinct, between, Column, String, Integer, func
# }}}
...
def remotedb_commonuse_create_table(SYSTEM):
    ...
    try:
        DbSession,metadata,engine= app.getDbSessionType(system=SYSTEM,forRawData="mssql")
        ...
        result ,err_msg = modules.create_table(table_name=table_name, attrList=table_attr, table_comment=table_comment, forRawData="mssql",system=SYSTEM)
...
def remotedb_commonuse_get_multi_rows_interval(SYSTEM,tableID):
    ...
    recList = []
    try:
        DbSession,metadata,engine= app.getDbSessionType(system=SYSTEM, forRawData="mssql", specified=specified)
        if DbSession is None:
            #表示連接資料庫有問題
            dicRet["Response"] = engine
            return jsonify( **dicRet)
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/ms/CommonUse/SqlSyntax/<tableName>', methods = ['POST']), 
...
def remotedb_commonuse_get_sqlsyntax(SYSTEM,tableName):
    ...
    try:
        ...
        query_table = Table(tableName , metadata, autoload=True)
        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,SYSTEM,selectDB="mssql")
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/2.0/ms/CommonUse/SqlSyntax', methods = ['POST']), 
...
def remotedb_commonuse_get_sqlsyntax_v2(SYSTEM):
    ...
    try:
        ...
        sqlsyntax_actions_status,sqlsyntax_actions_result = modules.sqlsyntax_params_actions(action_result,"condition_1",reqdataDict,sess,metadata,"mssql",False,SYSTEM)
    ...
    try:
        ...
        if fields != "":
            ...
            for fieldskey,fieldsvalue in fields.items():
                ...
                desc_table_return = modules.desc_table(metadata,fieldskey,SYSTEM,selectDB="mssql")
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
            ...
            for i in range(len(tables)):
                desc_table_return = modules.desc_table(metadata,tables[i],SYSTEM,selectDB="mssql")
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
                keyList = desc_table_return[2]
        ...
        for i in join[join.keys()[0]]:
            response, msg, key, value = modules.adjust_dict_for_joinapi(metadata=metadata,tables=tables,masterKey=joinkeyList0,data=i,joinkeyList=joinkeyList,joincolumnList=joincolumnList,selectDB="mssql",system=SYSTEM)
            if not response:
                dicRet["Response"] = msg
                return jsonify( **dicRet)
        else:
        ...
        if where != "":
            ...
            for wherekey,wherevalue in wheretoStrDict.items():
                ...
                #desc table schema
                desc_table_return = modules.desc_table(metadata,wherekey,SYSTEM,selectDB="mssql")
                desc_table_status = desc_table_return[0]
                fieldList = desc_table_return[1]
                keyList = desc_table_return[2]
                schemaDict = desc_table_return[3]
        ...
        if orderby != "":
            ...
            for i in range(1,len(orderby)):
                if i % 2 == 1:
                    ...
                    desc_table_return = modules.desc_table(metadata,orderby[i-1],SYSTEM,selectDB="mssql")
                    desc_table_status = desc_table_return[0]
                    fieldList = desc_table_return[1]
                    keyList = desc_table_return[2]
...
def remotedb_commonuse_get_sqlsyntax_joinmultitable_v2(SYSTEM):
    ...
    try:
        ...
        sqlsyntax_actions_status,sqlsyntax_actions_result = modules.join_sqlsyntax_params_actions(action_result,"condition_1",reqdataDict,sess,metadata,"mssql",True,SYSTEM)
        if not sqlsyntax_actions_status:
            dicRet["Response"] = sqlsyntax_actions_result
            return jsonify( **dicRet)
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/ms/CommonUse/<tableName>', methods = ['POST'])
...
def remotedb_commonuse_register(SYSTEM,tableName):
    ...
    try:
        ...
        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,SYSTEM,havetime=False,selectDB="mssql")
        desc_table_status = desc_table_return[0]
        fieldList = desc_table_return[1]
        keyList = desc_table_return[2]
        schemaDict = desc_table_return[3]
        ...
        if not keyList:
            HavePRI = False
            postDataKEYtostrList = modules.ConvertData().convert(reqdataDict)
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.5/ms/CommonUse/<tableName>', methods = ['POST'])
...
def remotedb_commonuse_register_v1_5(SYSTEM,tableName):
    ...
    try:
        ...
        result = modules.commonuse_register_integration_myms(reqdataDict, sess, metadata, tableName, SYSTEM, "mssql")
        err_msg = result[0]
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/ms/CommonUse/<tableName>', methods = ['PATCH'])
...
def remotedb_commonuse_update(SYSTEM,tableName):
    ...
    try:
        ...
        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,SYSTEM,havetime=True,selectDB="mssql")
        desc_table_status = desc_table_return[0]
        fieldList = desc_table_return[1]
        keyList = desc_table_return[2]
        schemaDict = desc_table_return[3]
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.5/ms/CommonUse/<tableName>', methods = ['PATCH'])
...
def remotedb_commonuse_update_v1_5(SYSTEM,tableName):
    ...
    try:
        ...
        result = modules.commonuse_update_integration_myms(reqdataDict, sess, metadata, tableName, SYSTEM, "mssql")
        err_msg = result[0]
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/ms/CommonUse/<tableName>', methods = ['DELETE'])
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.5/ms/CommonUse/<tableName>', methods = ['DELETE'])
...
def remotedb_commonuse_delete_v1_5(SYSTEM,tableName):
    ...
    try:
        ...
        result = modules.commonuse_delete_integration_myms(reqdataDict, sess, metadata, tableName, SYSTEM, "mssql")
        err_msg = result[0]
...
# FOR MSSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/ms/CommonUse/Schema/<tableName>', methods = ['GET'])
...
def commonuse_remotedb_update_temp(tableName):
    ...
    try: 
        #fill-in to object User for insertion
        DbSession,metadata,engine= app.getDbSessionType(system="IOT",forRawData="mssql")
        if DbSession is None:
            #表示連接資料庫有問題
            dicRet["Response"] = engine
            return jsonify( **dicRet)
        ...
        #check table existed or not
        if not modules.retrieve_table_exist(metadata,tableName,"IOT"):
            dicRet["Response"] = "Table '{}' doesn't exist".format(tableName)
            return jsonify( **dicRet)

        #desc table schema
        desc_table_return = modules.desc_table(metadata,tableName,"IOT",havetime=True,selectDB="mssql")
        desc_table_status = desc_table_return[0]
        fieldList = desc_table_return[1]
        keyList = desc_table_return[2]
        ...
        reqdataDict = json.loads(request.data)
        if isinstance(reqdataDict,type(u"")):
            reqdataDict = modules.AdjustDataFormat().format(json.loads(reqdataDict))
