...
function MainQuery($params)//主要查詢按鈕
{
    ...
    foreach ($Query_Device_Response as $key => $status_value) {
        ...
        // if (!isset($device_data['data'][$status_value['device_name']])) {
        //     $device_data['data'][$status_value['device_name']] = array('data' => []);
        // }

        $position = array_search($this_data_time[0] . '年' . $this_data_time[1] . '月', $abn_species['data_time']);
        // if (!isset($device_data['data'][$status_value['device_name']]['data'][$position])) {
        //     $device_data['data'][$status_value['device_name']]['data'][$position] = 0;
        // }
    ...
    $returnData['QueryTableData'][0]['query_date'] = [$params->startTime, $params->endTime, '二廠', $params->process == 5?'打頭':($params->process == 6?'輾牙':'--'), $params->process == 5?'成四組':($params->process == 6?'輾二組':'--'), $device_name];
