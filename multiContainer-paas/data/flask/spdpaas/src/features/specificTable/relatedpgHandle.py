...
def _check_idtype_createDB_ornot(reqdataDict, thisNum, tableName, metadata, sess, iscreatefunc=True):
    ...
    if noumenonidList[thisNum] == "null":
        ...
        thisid_existed,thisdbname = modules.retrieve_noumenon_dbname(metadata, sess, noumenontypeList[thisNum], noumenonidList[thisNum])
    ...
    if iscreatefunc:
        ...
        if createDBList[thisNum] == "yes":
            ...
            if modules.retrieve_database_exist(system, dbName=dbnameList[thisNum], forRawData="postgres")[0]:
                return False,"this 'db_name' : '{}' existed".format(dbnameList[thisNum])

            result,msg = modules.create_database(system,dbnameList[thisNum])
            if not result:
                return False,msg
...
def _transfer_table(SYSTEM,dbname,noumenon_sensorList,new_dbname):
    ...
    if success:
        try:
            ...
            #2.備份成功，刪除此資料表
            msg = modules.delete_tables(noumenon_sensorList,dbname,SYSTEM)
            if msg == "ok":
                try:
                    ...
                    try:
                        ...
                        app.logger_handler(SYSTEM,'Error', '<transfer_table_del_sqlfile>Unexpected error: {}'.format(e.output))
                except subprocess.CalledProcessError as e:
                    success = False
                    msg = "<transfer_table_backup_restore>Unexpected error: database : '{}' table-> '{}' 還原失敗".format(new_dbname,noumenon_sensorList)
                    app.logger_handler(SYSTEM,'Error', '<transfer_table_backup>Unexpected error: {}'.format(e.output))
            else:
                success = False
        except subprocess.CalledProcessError as e:
            success = False
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/RelatedPG/<tableName>', methods = ['POST'])
...
def relatedpg_register(SYSTEM,tableName):
    ...
    try:
        ...
        if createdbTrue and modules.retrieve_database_exist(SYSTEM, dbName=thisdbname, forRawData="postgres")[0]:
            modules.delete_database([thisdbname],SYSTEM)
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/RelatedPG/<tableName>', methods = ['PATCH'])
...
def relatedpg_update(SYSTEM,tableName):
    ...
    try:
        ...
        if reqdataDict.get("noumenon_type") is not None or reqdataDict.get("noumenon_id") is not None:
            if not modules.check_post_parameter_exist(reqdataDict,["noumenon_type","noumenon_id"]):
                                ...
                                if not result:
                                    ...
                                    if new_dbname is None:
                                        ...
                                        if postDataList["noumenon_id"] in noumenonDict[postDataList["noumenon_type"]]:
                                            ...
                                            #1.抓取舊的隸屬資料庫名稱
                                            thisid_existed,old_dbname = modules.retrieve_noumenon_dbname(metadata,sess,this_noumenon_type,this_noumenon_id)
                                            #2.先備份原本隸屬於此id的所有資料表，再刪除，然後還原至新的資料庫裡
                                            #3.判斷舊的資料庫名與新的隸屬資料庫名不同且old_dbname是否存在，判斷是否有要轉移的下階關係列表
                                            if noumenon_sensorList and (old_dbname != new_dbname) and modules.retrieve_database_exist(SYSTEM, dbName=old_dbname, forRawData="postgres")[0]:
...
# FOR MYSQL
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/my/RelatedPG/<tableName>', methods = ['DELETE'])
        ...
        for i in range(len(whereDict)):
            ...
            if check_data_existed:
                ...
                for key,value in noumenonDict.items():
                    for i in value:
                        _,catch_dbname = modules.retrieve_noumenon_dbname(metadata,sess,key,i)
                ...
                if this_dbname is not None:
                    ...
                    if list(set(delete_dbnameList).union([this_dbname])):
                        resultmsg = modules.delete_database(list(set(delete_dbnameList).union([this_dbname])),SYSTEM)
                        if resultmsg != "ok":
                    ...
                    #抓取此id的隸屬資料庫
                    thisid_existed,this_dbname = modules.retrieve_noumenon_dbname(metadata,sess,this_noumenon_type,this_noumenon_id)
                    #刪除下階的資料庫
                    #差集 set(b)-set(a) | set(b).difference(a)
                    if list(set(delete_dbnameList).difference([this_dbname])):
                        resultmsg = modules.delete_database(list(set(delete_dbnameList).difference([this_dbname])),SYSTEM)
                        if resultmsg != "ok":
                            deletestatus.append("Error '{}'".format(resultmsg))
                            thisdelete_fail = True
                        else:
                ...
                if not thisdelete_fail:
                    ...
                    if noumenon_sensorList: modules.delete_tables(noumenon_sensorList,this_dbname,SYSTEM)
