...
from apiPortal import app, socketio, threading, lock
import globalvar
import modules
        ...
        while self.switch:
            ...
            if self.is_uWSGI:
                ...
                if _monitorLogging["already_print_num"+self.filename] < datacounts:
                    ...
                    for i in range(len(print_lines)):
                        thisdata = modules.AdjustDataFormat().format(print_lines[i])
                        returnnum = _monitorLogging["already_print_num"+self.filename]+i+1
                        socketio.emit(this_socketio_name,
    ...
    def start(self,filename,system,is_uWSGI):
        ...
        if self.init:
            ...
            if self.is_uWSGI:
                ...
                try:
                    DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(system="PaaS",dbName="paas_dashboard",forRawData="postgres")
