...
def applogger(app):
    ...
    for i in tempsystemlist:
        if not os.path.isdir("/var/www/spdpaas/log/{}".format(i)):
            os.mkdir("/var/www/spdpaas/log/{}".format(i))

        local_variables["rotateLogFile_{}".format(i)] = os.path.join('/var/www/spdpaas/log/{}'.format(i),"log_%s"%(app.name))
    ...
    app.logger_handler = logger_handler
...
def catch_exception(exception, sys_exc_info, system, doLoggerHandler=True):
    ...
    for logtype in regexDict.keys():
        if re.search(regexDict[logtype],str(error_class_name)):
            thislogtype = logtype
    ...
    sysmessage = modules.AdjustDataFormat().format(sysmessage)
...
class DatabaseLoggingHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            if not modules.retrieve_database_exist(system=self.system,dbName="paas_dashboard",forRawData="postgres")[0]:
                modules.create_database(self.system,"paas_dashboard",forRawData="postgres")

            DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(dbName="paas_dashboard", system=self.system, forRawData="postgres")
            if not DbSessionRaw is None:
                ...
                if not modules.retrieve_table_exist(metaRaw,tableName,self.system,doLoggerHandler=False):
                    resultRaw ,mesgRaw = modules.create_table(table_name=tableName, dbName="paas_dashboard", attrList=table_attr, table_comment=self.system.lower()+"_PaaS API日誌",doLoggerHandler=False,system=self.system)
                    # print "~~~resultRaw~~~~"
                    # print resultRaw
                    # print "~~~mesgRaw~~~~"
                    # print mesgRaw
