...
__all__ = (
    ...
    'retrieve_database_exist','retrieve_table_exist','desc_table',
    'check_user_defined_type','adjust_dict_for_joinapi','create_database',
    'create_table','delete_database','delete_tables',
    'retrieve_noumenon_dbname','decorator_check_content_type',
    'check_post_parameter_exist','docstring_parameter'
)

# from apiPortal import threading
#}}}
...
# AES = modules.Prpcrypt(app.dicConfig.get('aes_key'),AES_IV)
...
class Prpcrypt():
    ...
    def encrypt(self, text):
        try:
            ...
            return False,app.catch_exception(e,sys.exc_info(),self.system)
...
class ApiSqlsyntaxActions():
    ...
    def __equal(self):
        ...
        if self.data["num"] == 0:
            return or_(getattr(self.data["query_table"].c,self.data["key"]) == self.data["value"])
        return or_(self.data["wheresql"],or_(getattr(self.data["query_table"].c,self.data["key"]) == self.data["value"]))
    
    def __is(self):
        if self.data["num"] == 0:
            return or_(getattr(self.data["query_table"].c,self.data["key"]).is_(None))
        return or_(self.data["wheresql"],or_(getattr(self.data["query_table"].c,self.data["key"]).is_(None)))
    ...
    def __isnot(self):
        if self.data["num"] == 0:
            return or_(getattr(self.data["query_table"].c,self.data["key"]).isnot(None))
        return or_(self.data["wheresql"],or_(getattr(self.data["query_table"].c,self.data["key"]).isnot(None)))
    ...
    def __like(self):
        if self.data["symbol"] == "like":
            thisvalue = "%{}%".format(self.data["value"])
        elif self.data["symbol"] == "leftlike":
            thisvalue = "%{}".format(self.data["value"])
        elif self.data["symbol"] == "rightlike":
            thisvalue = "{}%".format(self.data["value"])
            
        if self.data["db"] == "mssql" and (self.data["schemaDict"][self.data["key"]] in ("TIMESTAMP","DATETIME")):
            if self.data["num"] == 0:
                return or_(text("CONVERT(varchar(512),{},121) LIKE '{}'".format(getattr(self.data["query_table"].c,self.data["key"]),thisvalue)))
            return or_(self.data["wheresql"],or_(text("CONVERT(varchar(512),{},121) LIKE '{}'".format(getattr(self.data["query_table"].c,self.data["key"]),thisvalue))))

        if self.data["db"] == "postgres" and (self.data["schemaDict"][self.data["key"]] in ("TIMESTAMP WITHOUT TIME ZONE","INTEGER")):
            if self.data["num"] == 0:
                return or_(text("{} ::text LIKE '{}'".format(getattr(self.data["query_table"].c,self.data["key"]),thisvalue)))
            return or_(self.data["wheresql"],or_(text("{} ::text LIKE '{}'".format(getattr(self.data["query_table"].c,self.data["key"]),thisvalue))))

        if self.data["num"] == 0:
            return or_(getattr(self.data["query_table"].c,self.data["key"]).like("{}".format(thisvalue)))
        return or_(self.data["wheresql"],or_(getattr(self.data["query_table"].c,self.data["key"]).like("{}".format(thisvalue))))
    ...
    def __in(self):
        if self.data["num"] == 0:
            return or_(getattr(self.data["query_table"].c,self.data["key"]).in_(self.data["value"]))
        return or_(self.data["wheresql"],or_(getattr(self.data["query_table"].c,self.data["key"]).in_(self.data["value"])))

    def __notin(self):
        if self.data["num"] == 0:
            return or_(getattr(self.data["query_table"].c,self.data["key"]).notin_(self.data["value"]))
        return or_(self.data["wheresql"],or_(getattr(self.data["query_table"].c,self.data["key"]).notin_(self.data["value"])))

    #子查詢可選擇要and還or
    def __subin(self):
        # print self.data["value"]
        # print self.data["key"]
        # this_in_valueList = [ivalue for i in self.data["value"] for ikey,ivalue in i.items()]
        # print this_in_valueList
        # return and_(getattr(self.data["query_table"].c,self.data["key"]).in_(this_in_valueList).subquery())
        queryConfig = {
            "and":and_,
            "or":or_
        }
        if self.data["num"] == 0:
            return queryConfig[self.data["subquery_cond"]](getattr(self.data["query_table"].c,self.data["key"]).in_(self.data["value"]))
        else:
            return queryConfig[self.data["subquery_cond"]](self.data["wheresql"],queryConfig[self.data["subquery_cond"]](getattr(self.data["query_table"].c,self.data["key"]).in_(self.data["value"])))
    ...
    def __subnotin(self):
        ...
        if self.data["num"] == 0:
            return queryConfig[self.data["subquery_cond"]](getattr(self.data["query_table"].c,self.data["key"]).notin_(self.data["value"]))
        else:
            return queryConfig[self.data["subquery_cond"]](self.data["wheresql"],queryConfig[self.data["subquery_cond"]](getattr(self.data["query_table"].c,self.data["key"]).notin_(self.data["value"])))
    #}}}
...
class _WhereSqlsyntaxActions():
    ...
    def apiconditionschema(self,this):
        try:
            ...
            if not (isinstance(self.symbolstoStrDict[self.key][this],str) and (self.symbolstoStrDict[self.key][this].split("_")[0] in globalvar.SQLSYNTAX_SUBQUERY_SYMBOL.keys())):
                return False,"子查詢的symbol : '{}' 必須為字串且必須為以下其中一個 : {}".format(self.symbolstoStrDict[self.key][this].split("_")[0],globalvar.SQLSYNTAX_SUBQUERY_SYMBOL.keys())
        ...
        try:
            for c in range(len(self.value)):
                ...
                if self.apicondition:
                    ...
                    thissymbol = globalvar.SQLSYNTAX_SUBQUERY_SYMBOL[self.symbolstoStrDict[self.key][c].split("_")[0]]
                    subquery_cond = "and"
                    if len(self.symbolstoStrDict[self.key][c].split("_")) > 1:
                        subquery_cond = self.symbolstoStrDict[self.key][c].split("_")[1]
                else:
                    ...
                    if (not self.value[c] in ("null","Null","NULL")) and self.schemaDict[self.key] in ("TIMESTAMP","DATETIME","TIMESTAMP WITHOUT TIME ZONE"):
                        ...
                        if not VerifyDataStrLawyer(self.value[c]).verify_date():
                            return False,"error 'where' parameter input, date str '{}' is illegal".format(self.value[c])

                    if self.symbolstoStrDict[self.key][c] not in globalvar.SQLSYNTAX_OPERATOR:
                        return False,"error 'symbols' parameter input '{}', not in {}" .format(self.symbolstoStrDict[self.key][c],globalvar.SQLSYNTAX_OPERATOR)
                    
                    if self.symbolstoStrDict[self.key][c] in ("in","notin") and not isinstance(self.value[c],list):
                        return False,"error type of 'symbols' parameter input '{}', it must be an Array" .format(self.value[c])

                if c == 0:
                    whereor = ApiSqlsyntaxActions({"num":c,"query_table":self.query_table,"key":self.key,"symbol":thissymbol,"value":thisvalue,"wheresql":"","db":self.db,"schemaDict":self.schemaDict,"subquery_cond":subquery_cond}).sqlalchemy_symbol_condition()
                else:
                    whereor = ApiSqlsyntaxActions({"num":c,"query_table":self.query_table,"key":self.key,"symbol":thissymbol,"value":thisvalue,"wheresql":whereor,"db":self.db,"schemaDict":self.schemaDict,"subquery_cond":subquery_cond}).sqlalchemy_symbol_condition()
            else:
            ...
            for fieldskey,fieldsvalue in fields.items():
                ...
                for i in range(len(fieldsvalueList)):
                    if db == "mysql":
                        ...
                        tempfieldcolumnList.append("[{}].{} AS {}".format(fieldskey,fieldsvalueList[i],fieldskey+"$"+fieldsvalueList[i]))
            ...
            for i in range(len(tables)):
                ...
                for j in range(len(fieldList)):
                    if db == "mysql":
                        ...
                        tempfieldcolumnList.append("[{}].{} AS {}".format(tables[i],fieldList[j],tables[i]+"$"+fieldList[j]))
...
def check_dbconnect_success(sess, system, doLoggerHandler=True):
    ...
    try:
        ...
        err_msg = app.catch_exception(e, sys.exc_info(), system, doLoggerHandler=doLoggerHandler)
        status = False
    finally:
        sess.close()
...
def retrieve_database_exist(system, dbName="", forRawData="mysql"):
    ...
    if dbName == "":
        ...
        dbName = app.dicConfig.get(thisDB+"Dbname_"+system)
        # if forRawData == "mysql":
        #     dbName = app.dicConfig.get("dbiMgmDbEntity_SAPIDOSYSTEM")
        # elif forRawData == "mssql":
        #     dbName = app.dicConfig.get("dbiMssqlDbEntity_SAPIDOSYSTEM")
    try:
        DbSession,metadata,engine = app.getDbSessionType(dbName=dbName, forRawData=forRawData, system=system)
...
def retrieve_table_exist(metadata,tableName,system,doLoggerHandler=True):
    ...
    try:
        ...
        app.catch_exception(e,sys.exc_info(),system,doLoggerHandler=doLoggerHandler)
    finally:
        return tableexisted
#}}}
...
def create_database(system, dbName, forRawData="postgres"):
    ...
    try:
        ...
        DbSession,_,engine = app.getDbSessionType(dbName=default_dbName,forRawData=forRawData,system=system)
        print "~~~~~~~~~~"
        print DbSession
        print engine
        if DbSession is None:
...
def create_table(table_name, attrList, table_comment, system, dbName="", forRawData="postgres",doLoggerHandler=True):
    ...
    try:
        err_msg = "error"
        DbSession,metadata,engine = app.getDbSessionType(dbName=dbName,forRawData=forRawData,echo=True,system=system)
        if DbSession is None:
            #表示連接資料庫有問題
            return False,engine
...
def delete_database(dbName,system,forRawData="postgres"):
    ...
    try:
        DbSessionRaw,metadata,engineRaw = app.getDbSessionType(dbName="postgres",forRawData=forRawData,system=system)
...
class DecoratorOauth2ConnectDB(object):
    ...
    def __call__(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            print func.__name__
            try:
                DbSession,metadata,engine= app.getDbSessionType(system="PaaS",echo=True)
                if DbSession is None:
    ...
    try:
        ...
        for i in range(len(keyList)): 
            ...
            if reqdataDict.get(keyList[i]) is None:
                return ["primary key is necessary"]
            ...
            for j in range(postDataCount):
                ...
                if tableName not in ("baccount","user") and keyList[i] == "id":
                    ...
                    if postDataKEYtostrList[j] != "":
                        return ["primary key '{}'為自動地增值，放空值即可".format(keyList[i])]
            ...
            if not check_data_existed:
                for x in range(len(fieldList)):
                    ...
                    if (reqdataDict.get(fieldList[x]) is None) and (not defaultDict[fieldList[x]]):
                        return ["{} missing post some fields : '{}'".format(tableName,fieldList[x])]
        ...
        if len(insertstatus) != 0:
            return ["ok", "Error: {} ID ({}) existed".format(tableName," , ".join(insertstatus))]
        return ["ok"]

    except Exception as e:
        return [app.catch_exception(e,sys.exc_info(),SYSTEM)]
#}}}
