...
def transitionDB_commonuse_get_sqlsyntax_joinmultitable_v2(SYSTEM):
    ...
    try:
        ...
        sqlsyntax_actions_status,sqlsyntax_actions_result = join_modules.sqlsyntax_params_actions(action_result,"condition_1",reqdataDict,sess,metadata,"mysql",True,SYSTEM)
        if not sqlsyntax_actions_status:
            dicRet["Response"] = sqlsyntax_actions_result
            return jsonify( **dicRet)
...
#    #appPaaS.logging.info("{} {} from {}".format(request.method,request.path,request.args.get("user_id","undefined")))
...
#        appPaaS.sessionLogger_SAPIDOSYSTEM('Error','in Get Session, []: {}\n'.format(err_msg))
#        appPaaS.sessionLogger_SAPIDOSYSTEM('Error','get_session_id() check parameter fail. {}\n'.format(err_msg))
#    else:
...
#        print appPaaS.dicConfig.get('key_loc')
#        print "==============="
#        sig_check_ok, cal_signature = web_utility.check_signature(\
#                                      request.args['signature'], \
#                                      string_to_sign, \
#                                      appPaaS.dicConfig.get('key_loc'),
#                                      appPaaS.dicConfig.get('log_path'))
#        print "~~sig_check_ok~~"
#        print sig_check_ok
#        print "~~cal_signature~~"
#        print cal_signature
...
#            appPaaS.sessionLogger_SAPIDOSYSTEM('Error','get_session_id(): {}\n'.format(err_msg))
#            appPaaS.sessionLogger_SAPIDOSYSTEM('Error','client sent request.args:{}\n'.format(request.args))
#            appPaaS.sessionLogger_SAPIDOSYSTEM('Error','string_to_sign = {}\n'.format(string_to_sign))
#            appPaaS.sessionLogger_SAPIDOSYSTEM('Error','server calcaluted sign is {}\n'.format(cal_signature))
#            appPaaS.sessionLogger_SAPIDOSYSTEM('Error','client sent sign is {}\n'.format(request.args['signature'] ))
#
...
#            appPaaS.sessionLogger_SAPIDOSYSTEM('Error',strDebugMsg)
#            dicRet['debug_msg'] = strDebugMsg
#        else:
#            current_timestamp = request.args['timestamp']
#            #wei@03142017, adding uid pwd process thx DB
#            #pwd = appPaaS.dicConfig.get('user').get(request.args['uid'])
#            pwd = "1218test"
#            #appPaaS.logging.error('Line 480: uid: {}, passwd: {}\n'.format(request.args['uid'], pwd))
#            token_check_ok, err = authentic_utility.check_token(appPaaS.dicConfig.get('key_loc'), \
#                                        appPaaS.dicConfig.get('code'),   \
#                                        str(request.args['token']), \
#                                        request.args['uid'], \
#                                        #dicConfig.get('secret'),   \
#                                        pwd, \
#                                        current_timestamp, \
#                                        appPaaS.dicConfig.get('timeout'),   \
#                                        appPaaS.dicConfig.get('log_path'))
...
#            #pwd = appPaaS.dicConfig.get('user').get(request.args['uid'])
#            pwd = "1218test"
#            #appPaaS.logging.error('Line 498: uid: {}, pwd: {}\n'.format(request.args['uid'], pwd))
#            #session_id_str = dicConfig.get('secret') + '*' + current_timestamp
#            session_id_str = pwd + '*' + current_timestamp
#            print "~~session_id_str~~"
#            print session_id_str
#            encrypted_session_id = authentic_utility.encrypt_pgp(appPaaS.dicConfig.get('key_loc'), \
#                                                                    session_id_str, \
#                                                                    appPaaS.dicConfig.get('log_path') )
