...
function MainQuery($params)//主要查詢按鈕
{
    ...
    foreach ($Query_Device_Response as $key => $status_value) {
        ...
        //計算當日稼動率
        $this_activation = round($this_run_day_time / 86400, 2) * 100;
    ...
    $returnData['QueryTableData'][0]['query_date'] = [$params->startTime, $params->endTime, '二廠', $params->process == 5?'打頭':($params->process == 6?'輾牙':'--'), $params->process == 5?'成四組':($params->process == 6?'輾二組':'--'), $params->deviceID];
    $returnData['Response'] = 'ok';
    return  $returnData;
    // $returnData['QueryTableData'] = [];
    // $returnData['QueryTableData'][0]['activation'] = [];
    // if ($user_start_time <= strtotime("2020-09-22 00:00:00") && strtotime($params->endTime) >= strtotime("2020-09-22 23:59:59")) {
    //     array_push($returnData['QueryTableData'][0]['activation'], 
    //         array(
    //             'date' => '2020/09/22',//日期
    //             'site' => '久陽二廠',//廠區
    //             'group' => '成四組',//組別
    ...
    //     );
    // }
    // if ($user_start_time <= strtotime("2020-09-23 00:00:00") && strtotime($params->endTime) >= strtotime("2020-09-23 23:59:59")) {
    //     array_push($returnData['QueryTableData'][0]['activation'], 
    //         array(
    //             'date' => '2020/09/23',//日期
    //             'site' => '久陽二廠',//廠區
    ...
    //     );
    // }
    // if ($user_start_time <= strtotime("2020-09-24 00:00:00") && strtotime($params->endTime) >= strtotime("2020-09-24 23:59:59")) {
    //     array_push($returnData['QueryTableData'][0]['activation'], 
    //         array(
    //             'date' => '2020/09/24',//日期
    //             'site' => '久陽二廠',//廠區
    ...
    // if (empty($returnData['QueryTableData'][0]['activation'])) {
    //     $returnData['Response'] = 'no data';
    // } else {
