            ...
            for key2,value2 in value.items():
                if key2 not in globalvar.DBCONFIG.keys():
                    ...
                    for i in repeatconfig:
                        status,result = _getConfigParserDetails(CONFIG,value2,i)
                        if status:
                            diff_system[globalvar.DBCONFIG[key2]+i.capitalize()+"_"+key] = result
                        else:
                            diff_system[globalvar.DBCONFIG[key2]+i.capitalize()+"_"+key] = DEFAULT_DB_CONFIG[globalvar.DBCONFIG[key2]+i.capitalize()]
...
def getDbSessionType(dbName="", forRawData="mysql", system=None, driver="pyodbc", echo=False):
    ...
    #MYSQL
    MysqlUser = "DBMYSQLUser_"+system
    MysqlPassword = "DBMYSQLPassword_"+system
    MysqlIP = "DBMYSQLIp_"+system
    MysqlPort = "DBMYSQLPort_"+system
    MysqlDbname = "DBMYSQLDbname_"+system
    ...
    #Mssql
    MssqlUser = "DBMSSQLUser_"+system
    MssqlPassword = "DBMSSQLPassword_"+system
    MssqlIP = "DBMSSQLIp_"+system
    MssqlPort = "DBMSSQLPort_"+system
    MssqlDbname = "DBMSSQLDbname_"+system
    ...
    try:
        ...
        check_connect_success,check_result = modules.check_dbconnect_success(dbSessionType(), system)
