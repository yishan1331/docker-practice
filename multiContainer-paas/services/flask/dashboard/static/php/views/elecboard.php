...
function SingleQuery($params) //單機台查詢按鈕
{
    ...
    if (isset($machine_emeter['RELEC'])) {
        ...
        if ($this_month>=6 && $this_month<=9) {
            ...
            $day_electricityBill = round($electricityBill / $this_total_day + $this_day_RELEC * 2.58 * (1 + $adjust_rate));
            $week_electricityBill = round($electricityBill / 7 + $this_week_RELEC * 2.58 * (1 + $adjust_rate));
            $month_electricityBill = round($electricityBill + $this_month_RELEC * 2.58 * (1 + $adjust_rate));
        } else {
            $electricityBill = 60 * 173.20;
            $day_electricityBill = round($electricityBill / $this_total_day + $this_day_RELEC * 2.45 * (1 + $adjust_rate));
            $week_electricityBill = round($electricityBill / 7 + $this_week_RELEC * 2.45 * (1 + $adjust_rate));
            $month_electricityBill = round($electricityBill + $this_month_RELEC * 2.45 * (1 + $adjust_rate));
        }
