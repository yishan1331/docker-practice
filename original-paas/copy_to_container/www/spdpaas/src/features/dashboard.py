...
from apiPortal import socketio, app
import globalvar
import modules
print "=====dashboard.py======"
...
def get_logtype(SYSTEM):
    ...
    try:
        for key,value in globalvar.LOGFILELIST.items():
            FileList[key] = []
            if key == "PaaS_LOG":
                DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(dbName="paas_dashboard",forRawData="postgres",system="PaaS")
                ...
                # if modules.retrieve_table_exist(metaRaw,SYSTEM.lower()+"_paas_log_"+time.strftime("%Y%m", time.localtime()),SYSTEM): File.append(key)
            else:
                File.append(key)
                if GetLogFilesList:
...
def get_apicountedtable(SYSTEM):
    ...
    try:
        # if not modules.retrieve_database_exist(SYSTEM, dbName=dbName, forRawData="postgres"):
        #     dicRet["Response"] = "此系統:{} 尚未有api counted table".format(SYSTEM)
        #     return jsonify( **dicRet)

        DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(dbName=dbName,forRawData="postgres",system="PaaS")
        if DbSessionRaw is None:
            #表示連接資料庫有問題
            dicRet["Response"] = engineRaw
            return jsonify( **dicRet)
...
def get_apicountedtabledata(TableName):
    ...
    try:
        ...
        if not modules.retrieve_table_exist(metaRaw,TableName,SYSTEM):
...
def daily_calculate_aggregately_apirecords():
    ...
    try:
        dbRedis,_,result= app.getDbSessionType(system="PaaS",dbName=15,forRawData="redis")
        if dbRedis is None:
            #表示連接資料庫有問題
        ...
        for i in tempsystemlist:
            ...
            for yesterday_redis_key in dbRedis.keys(yesterday_redis_keys):
                if dbRedis.exists(yesterday_redis_key):
                    ...
                    print dbName
                    try:
                        DbSessionRaw,metaRaw,engineRaw = app.getDbSessionType(system="PaaS",dbName=dbName,forRawData="postgres")
                        if DbSessionRaw is None:
                            #表示連接資料庫有問題
                            dicRet["Response"] = engineRaw
                            return jsonify( **dicRet)
