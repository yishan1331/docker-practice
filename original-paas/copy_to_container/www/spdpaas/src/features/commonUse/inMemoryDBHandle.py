...
class _RedisInsertUpdateAction(object):
    def __init__(self,dbRedis,datatype,data,whichAction,setExpireTime):
        ...
        self.datatype = datatype
        self.data = data
        self.whichAction = False
        self.doAction = "update"
        if whichAction == "Insert":
            self.whichAction = True
            self.doAction = "add"
    ...
    def _list_set(self):
        try:
            for key,value in self.data["data"].items():
                if self.dbRedis.exists(key):
                    if self.whichAction:
                        self.err_msg.append("Failed to add Key:'{}' because already exists,".format(key))
                        continue
                ...
                if not self.whichAction: self.dbRedis.delete(key)
...
# FOR REDIS
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/rd/CommonUse/Keys', methods = ['POST'])
...
def redis_commonuse_register(SYSTEM):
    ...
    reqdataDict = modules.ConvertData().convert(json.loads(request.data))
    ...
    try:
        ...
        for this_key_type in redis_key_type:
            if reqdataDict.get(this_key_type) is not None:
                anydata = True
                if not modules.check_post_parameter_exist(reqdataDict.get(this_key_type),["data"]):
...
# FOR REDIS
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/rd/CommonUse/Keys', methods = ['PUT'])
@INMEMORYDB_API.route('/api/<SYSTEM>/1.0/rd/CommonUse/Keys', methods = ['PUT'])
def redis_commonuse_update_all_value(SYSTEM):
    #{{{APIINFO
    '''
    {
    ...
    try:
        ...
        for this_key_type in redis_key_type:
            if reqdataDict.get(this_key_type) is not None:
                ...
                if "expire_time" in reqdataDict.get(this_key_type).keys(): setExpireTime = True
                #若key有要設定有效期限，則一筆一筆新增(較慢)，否則一次新增(較快)
                updatestatus[this_key_type] = _RedisInsertUpdateAction(dbRedis,this_key_type,reqdataDict.get(this_key_type),"Update",setExpireTime).set_value()
        else:
            if not anydata:
                dicRet["Response"] = "Faild to add,because no correct data"
                return jsonify( **dicRet)
...
# FOR REDIS
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/rd/CommonUse/Hash/Keys/SpecificField', methods = ['PATCH'])
...
def redis_commonuse_hash_update_specific_field(SYSTEM):
    ...
    is_illegal,is_dict = modules.VerifyDataStrLawyer(request.data).verify_json(check_dict=True)
...
# FOR REDIS
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/rd/CommonUse/Keys', methods = ['DELETE'])
...
def redis_commonuse_delete(SYSTEM):
    ...
    if not modules.check_post_parameter_exist(reqdataDict,["keys"]):
...
#=======================================================
#{{{ INMEMORYDB_API.route('/api/<SYSTEM>/1.0/rd/CommonUse/Interval/<key>', methods = ['GET']), 
@INMEMORYDB_API.route('/api/<SYSTEM>/1.0/rd/CommonUse/SpecificKey/<key>/<number>',  methods = ['GET'])
def test_redis_commonuse_get_specific_key(SYSTEM, key, number):
    #{{{APIINFO
    '''
    {
        "API_application":"提供查詢redis資料庫指定key的值，若pattern為yes表示欲使用*、[]進行條件匹配查詢，例如：h*llo匹配hllo和heeeeello；h[ae]llo匹配hello和hallo，但不匹配hillo",
...
#=======================================================
# {{{ app.route('/api/<SYSTEM>/1.0/rd/CommonUse/Hash/Keys/SpecificField', methods = ['PATCH'])
@INMEMORYDB_API.route('/api/<SYSTEM>/1.0/rd/CommonUse/Hash/Keys/SpecificField/<number>', methods = ['PATCH'])
def test_redis_commonuse_hash_update_specific_field(SYSTEM, number):
    #{{{APIINFO
    '''
    {
        "API_application":"提供修改redis Hash型態key的指定field值，為部分更新",
