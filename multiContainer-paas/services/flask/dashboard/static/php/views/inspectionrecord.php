...
function download_inspection_file($params)
{
    ...
    foreach ($sensor_code_array as $key => $value) {
        if ($key == 'vibration') {
            ...
            $loadCell_fields = array();
            $main_fields = array();
            foreach ($value['sensor_code'] as $sensor_code) {
                if ($sensor_code == 'wire_weight') {
                    array_push($loadCell_fields, $sensor_code);
                } else if ($sensor_code == 'mcg_revsp') {
                    array_push($main_fields, $sensor_code);
                }
            }

            //loadCell
            if (!empty($loadCell_fields)) {
                $file_name_loadCell = $start_time . '~' . $end_time . 'deviceStatus-loadCell';
                array_push($loadCell_fields, 'upload_at');
                $data = array(
                    'fields' => $loadCell_fields,
                    'intervaltime' => array(
                        'upload_at' => [$selected_date->startTime,  $selected_date->endTime]
                    ),
                    'exportfilename' => $file_name_loadCell
                );
                $inspection_result = ExportCsv($data, 'PostgreSQL', $cus_id . '_' . $device_id . '_loadCell');
                if ($inspection_result['Response'] == 'ok') {
                    array_push($file_name_array, $file_name_loadCell);
                }
            }

            //main
            if (!empty($main_fields)) {
                $file_name_main = $start_time . '~' . $end_time . 'deviceStatus-main';
                array_push($main_fields, 'upload_at');
                $data = array(
                    'fields' => $main_fields,
                    'intervaltime' => array(
                        'upload_at' => [$selected_date->startTime,  $selected_date->endTime]
                    ),
                    'exportfilename' => $file_name_main
                );
                $inspection_result = ExportCsv($data, 'PostgreSQL', $cus_id . '_' . $device_id . '_main');
                if ($inspection_result['Response'] == 'ok') {
                    array_push($file_name_array, $file_name_main);
                }
            }
        } else {
